<?
	...
	// Оқушыны қайта кіргізу
			...
			$userId = $_POST["userId"];
   			$result = $mysqli->query("SELECT `userId` FROM `users` WHERE `id`='".$userId."'")->fetch_assoc();
			$c = $result["userId"];
			for($i = 0; $i < 10; $i++){
				$c = md5($c);
			}
   			$mysqli->query("UPDATE `users` SET `status` = '0', `joinToGroup`='0', `password`='".$c."' WHERE `id`='".$userId."'");
			echo $result["userId"];
			$mysqli->close();
		}
	...
	// Баға қою.
			...
			$text = "";
			if($wam){
				if($bool2 == 1) $text = $wam["text"] . "|" . $allResult;
				else {
					$mWam = explode("|", $wam["text"]);
					for($i = 0; $i < count($mWam) - 1; $i++){
						$text = $text . $mWam[$i] . "|";
					}
					$text = $text . $allResult;
				}
			...
			foreach ($m as $key => $value) {
				...
				$bal = "";
				if($bool2 == 1) { $bal = $result["results"]. $c . date("d-m") . ";" .$topic . ";" .$totalOf100 . " " . $totalOf125;}
				else{
					$user_results = explode(":", $result["results"]);
					for($i = 0; $i < count($user_results) - 1; $i++){
						$bal = $bal . $user_results[$i] . ":";
					}
					$bal = $bal . date("d-m") . ";" .$topic . ";" .$totalOf100 . " " . $totalOf125;
				}
	...
	// Search.
			...
			$text = $_POST['text'];
			$result = $mysqli->query("SELECT `id`, `name`,`subject`,`tel`,`userId`,`tel2`,`tel3`,`group`, `money`,`datesPaid`, `pay`, `paid` FROM `users` WHERE `name` LIKE '%".$text."%' AND `who` = '1' AND `status` = '0'");
			$m = array();
			$i = 0;
	...
	// add news
			...
			$topic = $request->topic;
			echo date("H:i d.m.y");
			$mysqli->query("INSERT INTO `news` (`topic`, `text`, `date`) VALUES ('".$topic."', '".$text."', '".date("Y:m:d - H:i:s")."')");
			$mysqli->close();
		}
?>
