<?
	...
	if($_SESSION["who"] != 2) exit(header("Location: main.php"));
	...
	<div class = "middle">
					...
					if($result3["results"] != 12){
						...
						while(($row = $result2->fetch_assoc()) != false){
							...
							echo '<tr class="tr2 " id = "'.$row['id'].'">
									...
									for($j = 0; $j < $number; $j++){
										if($j >= $allNum){
											$bal = explode(";", $bals[$j -$allNum	 ]);
											$date = $bal[0];
											$totals = explode(" ", $bal[2]);
										}
		...
		?>
			<style>
				...
				.editBagaCheckbox input{margin-right: 10px;line-height: 15px;}
				.editBagaCheckbox label{line-height: 15px;}
				.editBagaSave{display: none;margin: 5px;}
			</style>
			<script>
				$(document).ready(function(){
					$("#editBagaCheckbox").click(function(){
						if($(this).is(":checked")){$(".editBagaSave").show(500);}
						else {$(".editBagaSave").hide(500);}
					});
				});
			</script>
			<!-- Оқушылармен жұмыс -->
			<div class = "Hide workWithCuratorDiv BagaClose">
					<div class="editBagaCheckbox">
						<input type="checkbox" id = "editBagaCheckbox"><label for="editBagaCheckbox">Редактировать</label>
					</div>
				...
				<div class = "fromCurator">
					...
					<table class="t2 Table">
						...
						<button class = "btn btn-primary editBagaSave">Сохранить</button>
				</div>
				<div class = "dialog">
					<div class = "headD"></div>
					<div class = "bodyD"></div>
