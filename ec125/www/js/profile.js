$(document).ready(function(){
	...
	// Оқушыны қайта кіргізу
		$(".backUser").click(function(){
			$.ajax({
				url: "queries.php",
				type: "POST",
				data:{bool:52, userId: $(this).get(0).id},
				dataType: "html",
				success: function(data){
					// Timeout();
					if(data){
						$(".span1").html("Успешно сделано.<br>Id - "+data+"<br>Пороль - "+data+"<br>Осы мәліметтерді оқушыға беріңіз");
						$(".span1").css("color","#00CD66");
						$(".errors").fadeIn(500);
					} else {
						$(".span1").text("Ошибка.");
	...
	// Оқушыны топқа қосу / Оқушының тобын ауыстыру.
		...
		//  Баға қою
			function setBal(input, p, id){
				...
				total100 = m[0] + m[1] + m[2] + m[3];
				total125 = m[0] + m[1] + m[2] + m[3] + m[4];
				$("#total100" + id).text("" + (total100));
				$("#total125" + id).text("" + (total125));
			}

			// Қойылған бағаларды сақтау
				function saveBaga(tId, plus_to_user_id){
					var bool = true;
					...
					for(var i = 1; i < m.length; i++){
						...
						var kl =$("#klP" + plus_to_user_id + userId).text();

						var math = $("#mP" + plus_to_user_id + userId).text();

						var history = $("#hP" + plus_to_user_id + userId).text();

						var rl = $("#rlP" + plus_to_user_id + userId).text();

						var sub5 = $("#s5P" + plus_to_user_id + userId).text();

						if(kl == "") {$("#klP" + plus_to_user_id + userId).css("color", "#fff");$("#klP" + plus_to_user_id + userId).parent().css("background", "#c9302c");bool = false;}
						if(math == "") {$("#mP" + plus_to_user_id + userId).css("color", "#fff");$("#mP" + plus_to_user_id + userId).parent().css("background", "#c9302c");bool = false;}
						if(history == "") {$("#hP" + plus_to_user_id + userId).css("color", "#fff");$("#hP" + plus_to_user_id + userId).parent().css("background", "#c9302c");bool = false;}
						if(rl == "") {$("#rlP" + plus_to_user_id + userId).css("color", "#fff");$("#rlP" + plus_to_user_id + userId).parent().css("background", "#c9302c");bool = false;}
						if(sub5 == "") {$("#s5P" + plus_to_user_id + userId).css("color", "#fff");$("#s5P" + plus_to_user_id + userId).parent().css("background", "#c9302c");bool = false;}
					...
					if(bool){
						$.ajax({
							url: "queries.php",
							type: "post",
							data: {bool:12, bool2:1, groupId:parseInt(tId), topic:topic,message:message, allResults:allResults},
							dataType: "html",
							success: function(data){
								if(data == 1){
									$(".clear").val("");
			...
			$(".blockLeft").delegate(".tB", "mouseenter", function(){
				p = $(this).children().get(0).id;
				input = $(this).children().get(1).id;
				$("#" + p).hide();
				$("#" + input).show();
			...
			$(".blockLeft").delegate(".tB", "mouseleave", function(){
				$(".tB .pB").show();
				$(".tB .writeB").hide();
				p = $(this).children().get(0).id;
				input = $(this).children().get(1).id;
					...
					success: function(data){
						if(data != false){
							...
							for(var i = 0; i < data.length; i++){
								var row = data[i];
								if(i + 1 == num) td = "td2l";
								$("#found2").append('<tr class="tr2"><td class="td2 td2f '+td+'">'+(i + 1)+'</td><td class="td2 '+td+' tdName"><a href = "profile1.php" class = "userName getName'+row['id']+'" id = "'+row['id']+'">'+row['name']+'</a></td><td class="td2 '+td+'">'+row["tel"]+'</td><td class="td2 '+td+'">'+row['tel2']+'</td><td class="td2 '+td+'">'+row['tel3']+'</td><td class="td2 '+td+'">'+row['group']+'</td><td class="td2 simvols2 '+td+'"><p class = "do1 late" id = "'+row['id']+'" title = "Келмеген оқушылар тізіміне қосу."><b>&#10143;</b></p><p id = "'+row['id']+'" class = "do2  willDelete" title = "Оқудан шығатын оқушылар тізіміне қосу.">&#10010;</p></td></tr>');
							}
						} else {
							$("#found2").html("");
							$(".found2").show();
	...
	// profile5 search2
		$(".fa-search").on("click", function(){
			...
			// Edit - Қойылған бағаларды сақтау
				$(".blockLeft").delegate(".tBEdit", "mouseenter", function(){
					if($("#editBagaCheckbox").is(":checked")){
						p = $(this).children().get(0).id;
						input = $(this).children().get(1).id;
						$("#" + p).hide();
						$("#" + input).show();
				...
				$(".blockLeft").delegate(".tBEdit", "mouseleave", function(){
					if($("#editBagaCheckbox").is(":checked")){
						$(".tBEdit .pB").show();
						$(".tBEdit .writeB").hide();
						p = $(this).children().get(0).id;
				...
				// Баға қойылған соң оны қайта өзгерту
				function updateBaga(tId, plus_to_user_id, oldBals){
					var bool = true;
					$(".tB").css("background", "#fff");
					$(".tB p").css("color", "#333");
					var allResults = "";
					...
					var mAllResults = allResults.split(":");
					var contains = false;
					for(var i = 0; i < mAllResults.length; i++){
						var user_bal = mAllResults[i].split(" ");
						if(parseInt(user_bal[0]) == parseInt(oldBals[i + 1][8])){
							for(var j = 0; j < 7; j++){
								if(parseInt(user_bal[j + 1]) != parseInt(oldBals[i + 1][j])){
									contains = true;
									break;
								}
							}
					...
					if(contains){
						$.ajax({
							url: "queries.php",
							type: "post",
							data: {bool:12, bool2:2, groupId:parseInt(tId), topic:topic,message:message, allResults:allResults},
							dataType: "html",
							success: function(data){
								if(data == 1){
									Timeout();
						...
						Timeout();
						$(".span1").text("Ешқандай өзгеріс енгізілген жоқ!");
						$(".span1").css("color","#EE6363");
						$(".errors").fadeIn(500);
					}
				}
			$(".editBagaSave").click(function(){
				...
				if($("#editBagaCheckbox").is(":checked")){
					$.ajax({
						...
						dataType:"json",
						success: function(data){
							updateBaga($(".editBagaSave").get(0).id, "edit", data);
						}
					});
				}
			});
					...
					$.ajax({
						...
						success: function(data){
							...
							if(data != false && data.length > 2){
								...
								$(".editBagaSave").get(0).id = groupId;
								console.log($(".editBagaSave").get(0).id, 456);
								...
								for(var i = 1; i < data.length - 2; i++){
									...
									oTotal125 += parseInt(total125);
									var editId = "edit"+id;
									$(".Table").append('<tr class="tr2"  id = "edit'+id+'">\
										<td class="td2 td2f '+td+'">'+i+'</td>\
										<td class="td2 '+td+' tdName"><p class = "NameTeacher" id = "'+id+'">'+name+'</p></td>\
										<td class="td2 tBEdit '+td+'"><p class = "pB '+editId+'" id = "klP'+editId+'">'+kl+'</p><input type="text" id = "klInput'+editId+'" class="writeB write clear" value = "'+kl+'"></td>\
										<td class="td2 tBEdit '+td+'"><p class = "pB '+editId+'" id = "mP'+editId+'">'+m+'</p><input type="text" id = "mInput'+editId+'" class="writeB write clear" value = "'+m+'"></td>\
										<td class="td2 tBEdit '+td+'"><p class = "pB '+editId+'" id = "hP'+editId+'">'+h+'</p><input type="text" id = "hInput'+editId+'" class="writeB write clear" value = "'+h+'"></td>\
										<td class="td2 tBEdit '+td+'"><p class = "pB '+editId+'" id = "rlP'+editId+'">'+rl+'</p><input type="text" id = "rlInput'+editId+'" class="writeB write clear" value = "'+rl+'"></td>\
										<td class="td2 tBEdit '+td+'"><p class = "pB '+editId+'" id = "s5P'+editId+'">'+sub5+'</p><input type="text" id = "s5Input'+editId+'" class="writeB write clear" value = "'+sub5+'"></td>\
										<td class="td2 '+td+'"><p class = "pB" id = "total100'+editId+'">'+total100+'</p></td>\
										<td class="td2 '+td+'"><p class = "pB" id = "total125'+editId+'">'+total125+'</p></td>\
										<td class="td2 '+td+' tdSubject"><p>'+subject+'</p></td></tr>');
								}
									oKl = (oKl / N).toFixed(2);
									oM = (oM / N).toFixed(2);
									oH = (oH / N).toFixed(2);
